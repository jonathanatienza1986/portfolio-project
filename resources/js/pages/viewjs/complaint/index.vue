...
onMounted(() => { // invoked when page ready
    ...
    search_filter.value = props.accountnumber;
...
const search_filter_clicked = () => {
    ...
    form_payload.get(complaint.index({ accountnumber: search_filter.value }).url, { //------------------------- force ajax parameter
        preserveScroll: true,
        preserveState: true,
    });
}
...
const new_complaint = () => {
    const form_payload = useForm();
    form_payload.get(complaint.create(form_payload.id).url, { 'id': form_payload.id });
}
</script>

<template>

    <Head title="complaint.index" />
    ...
    <div class=" z-0 h-full w-full overflow-auto p-0 m-0 justify-center">
        ...
        <div class="p-8 flex flex-col gap-3 w-full">
            ...
            <div class="flex flex-col lg:flex-row gap-2 p-2 lg:gap-6 lg:p-6 align-middle w-full border overflow-hidden">
                <Label>
                    Search Filter Account Number
                </Label>
                <div class="flex flex-row w-full">
                    <Input type="text" class=" rounded-none rounded-l-full" v-model="search_filter"
                        @change="search_filter_clicked" @keyup="search_filter_clicked" />
            ...
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <Button class=" bg-green-600 cursor-pointer" @click="new_complaint">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            ...
            <table ref="table_header">
                <tr class="align-middle text-justify p-8 border border-b-4">
                    <th class="p-4 text-lg">No.</th>
                    <th class="p-4 text-lg">Date/Time</th>
                    <th class="p-4 text-lg">Complaint</th>
                    <th class="p-4 text-lg">Description</th>
                    <th class="p-4 text-lg">Name</th>
                    <th class="p-4 text-lg">Coop No.</th>
                    <th class="p-4 text-lg">Address</th>
                    <th class="p-4 text-lg cursor-pointer">
                        <a :href="complaint.index().url">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                ...
                <tr class="align-middle text-justify p-8 border" v-for="(item, index) in complaints.data" :key="index">
                    <td class="p-4">{{ item.id }}</td>
                    <td class="p-4">{{ item.created_at.replace(".000000Z", "").replace("T", " ") }}</td>
                    <td class="p-4">{{ item.complaint }}</td>
                    <td class="p-4">{{ item.description }}</td>
                    <td class="p-4">{{ item.name }}</td>
                    <td class="p-4">{{ item.accountnumber }}</td>
                    <td class="p-4">{{ item.address }}</td>
                    <td class="p-4">
                        <a :href="complaint.show(item.id).url">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
            ...
            <div class="flex flex-row gap-2 p-2 lg:gap-6 lg:p-6 bg w-full border overflow-hidden">
                <div class=" text-blue-600" v-show="complaints.prev_page_url">
                    <a :href="complaints.prev_page_url">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                ...
                <div v-for="(item, index) in complaints.links" :key="index">
                    <a :href="item.url"
                        v-show="(index.toString() == item.label) && (complaints.current_page.toString() !== item.label.toString())">
                        {{ item.label }}</a>
                    <div
                        v-show="(index.toString() == item.label) && (complaints.current_page.toString() === item.label.toString())">
                        <span class=" underline font-bold text-lg">{{ item.label }}</span></div>
                </div>
                <div class=" mx-auto">
                    <span>{{ complaints.current_page }}&nbsp;out of&nbsp;{{ complaints.last_page }} Pages</span>
                </div>
                <div class=" text-blue-600" v-show="complaints.next_page_url">
                    <a :href="complaints.next_page_url">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
