...
const next_page = () => {
    ...
    isDisabled.value = true;
    const form_payload = useForm(form.value);
    form_payload.post(complaint.post().url, { 'id': form_payload.id });
};
defineOptions({
    layout: {
        breadcrumbs: [
...
const file_change = (data) => { // drivers licnese
    ...
    console.log("file_change: ", data.target.files[0]);
}
//--------------------------------------------------------------------------- dialog
import {
    Dialog,
...
const takePhoto = async () => { // drivers license
    ...
    form.value.image_file = new File([blob], "myimage12345.png", { type: blob.type, lastModified: Date.now() });
    console.log("takePhoto: ", form.value.image_file);
};
    ...
    <div class=" z-0 h-full w-full overflow-auto p-0 m-0 justify-center">
        ...
        <div class="p-8 flex flex-col w-full lg:w-[90%] gap-7">
            ...
            <div class="flex flex-row  mb-20 mt-10 pt-10 border-t">
                <Button :disabled="true" class=" cursor-pointer active:bg-amber-200 h-14" @click="prev_page">
