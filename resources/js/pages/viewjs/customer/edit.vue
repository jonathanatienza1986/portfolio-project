...
const on_mounted_or_ajax_post_success = () => { // interface between props, form and image_links
    // merge and update key:value pair of form from props data
    form.value = { ...form.value, ...props.customer };

    // re initialize image files in the form to lessen payload weight during upload
    form.value.license_image = null;
    form.value.govt_id_image = null;
    form.value.portrait_image = null;
    form.value.pic1_image = null;
    form.value.pic2_image = null;
    form.value.pic3_image = null;

    //brightness_changed();

    // assign image file link
    //file_base64.value = form.value.license_link; // = ref("");// drivers licnese
    //file_base641.value = form.value.portrait_link; // = ref("");// portrait photo
    //file_base642.value = form.value.govt_id_link; // = ref("");// other id
    //file_base643.value = form.value.pic1_link; // = ref("");// other picture 1
    //file_base644.value = form.value.pic2_link; // = ref("");// other picture 2
    //file_base645.value = form.value.pic3_link; // = ref("");// other picture 3
}
onMounted(() => { // invoked when page ready
    ...
    console.log("page.props.auth.user.id: ", page.props.auth.user.id);
    console.log("page.props.auth.user.email: ", page.props.auth.user.email);

    // envoke data management
    on_mounted_or_ajax_post_success();
});
...
const prev_page = () => {
    ...
    form_payload.get(customer.show(form.value.id).url, {
...
const next_page = () => {
    ...
    form_payload.post(customer.update(form.value.id).url, {
        //----------------------------------------- force ajax parameters
        preserveScroll: true,
        preserveState: true,
    });
...
// used for ajax updating for the to be uploaded images
const update_ajax = () => {
    console.log("form payload", form.value);
    const form_payload = useForm(form.value);
    // note: post with preserveScroll=true, will not remount the DOM
    //       instead, it will envoke onSuccess method
    form_payload.post(customer.update_ajax(form.value.id).url, {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
            // envoke data management
...
// --------------------------------------------------------------------------- Input file image/*
//const file_base64 = ref("");// drivers licnese
//const file_base641 = ref("");// portrait photo
//const file_base642 = ref("");// other id
//const file_base643 = ref("");// other picture 1
...
const brightness_changed = () => {
    ...
    img.src = form.value.license_link;
}

const fileScan = async () => {
    if (form.value.license_link) {
        brightness_changed();
        ...
        <div class="p-8 flex flex-col w-full lg:w-[90%] gap-7">
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="lg:w-[65%] flex flex-col gap-2 mb-0 border-2 rounded-xl min-w-72 overflow-hidden">
                    <img :src="form.license_link" alt="x" style="object-fit: cover; margin: auto;  height: 100%;" />
                </div>
                <div class="lg:w-[35%]">
                    ...
                    <div>
                        ...
                        <Dialog>
                            <DialogTrigger as-child>
                                <Button class="active:bg-amber-300 w-[100%]" @click="id_data.value = null">
                                    Scan ID
                                </Button>
                            </DialogTrigger>
                            <DialogContent class="w-[100%]">
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                Other Gov't ID
                <div class="lg:w-[65%] flex flex-col gap-2 mb-0 border-2 rounded-xl min-w-72 overflow-hidden">
                    <img :src="form.govt_id_link" alt="x" style="object-fit: cover; margin: auto;  height: 100%;" />
                </div>
                <div class="lg:w-[35%]">
                    <div class="w-full flex flex-col gap-2 mb-0">
                        <Label>
                            Upload other gov't ID</Label>
                        <div class="w-full h-10 rounded-lg mb-1 border overflow-hidden">
                    ...
                    <div class="w-full flex flex-col gap-2 mb-6">
                        ...
                        <Input type="text" name="govt_id_no" placeholder="ex. 90784389734789"
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                Portrait Photo
                <div class="lg:w-[65%] flex flex-col gap-2 mb-0 border-2 rounded-xl min-w-72 overflow-hidden">
                    <img :src="form.portrait_link" alt="x" style="object-fit: cover; margin: auto;  height: 100%;" />
                </div>
                <div class="lg:w-[35%]">
                    <div class="w-full flex flex-col gap-2 mb-0">
